...
class FDVarFormerLitModule(LightningModule):
    ...
    def __init__(
        ...
        weights_dict = torch.load(self.hparams.pred_ckpt, map_location=torch.device('cpu'))['state_dict']
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, mask: torch.Tensor, mult: torch.Tensor) -> torch.Tensor:
        """Perform a forward pass through the model `self.net`.

        :param x: A tensor of images.
        :return: A tensor of logits.
        """
        return self.net(x, y, mask, mult)
    ...
    def model_step(
        self, batch: Tuple[torch.Tensor, torch.Tensor], pred_step: int, phase: str
        ...
        with torch.set_grad_enabled(True):
            ...
            if pred_step > 0:
                ...
                for i in range(pred_step):
                    preds.append(self.net.phi_r(preds[-1]))
                    loss = loss + self.criterion(preds[i + 1], xt[:, i + 1])
                loss = loss / (pred_step + 1)
    ...
    def training_step(
        ...
        """
        loss, preds, targets = self.model_step(batch, 0, "train")
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        :param batch_idx: The index of the current batch.
        """
        loss, preds, targets = self.model_step(batch, 0, "val")
        ...
        self.log("val/rmse_r500", val_rmse[self.var_idx.index('r@500')], on_step=False, on_epoch=True, prog_bar=True)
    ...
    def validation_epoch_end(self, validation_step_outputs) -> None:
        ...
        self.val_acc_r500_best(val_acc_t850)  # update best so far val acc of r500
    ...
    def test_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        :param batch_idx: The index of the current batch.
        """
        loss, preds, targets = self.model_step(batch, 0, "test")
