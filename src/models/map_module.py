...
class MapLitModule(LightningModule):
    ...
    def __init__(
        ...
        clims = np.ones([3, self.net.out_chans, self.net.image_height, self.net.image_width])
        ...
        mask = np.zeros([1, 4, self.net.out_chans, self.net.image_height, self.net.image_width])
        for i in range(self.net.out_chans):
    ...
    def model_step(
        ...
        """
        xt, xb, obs_wind, obs_t, obs_r = batch
        ...
        xa = self.forward(xb,
                          new_obs[:,0].detach() * self.mask[:,0].to(xt.device, dtype=xt.dtype),
                          self.mask[:,0].to(xt.device, dtype=xt.dtype))
