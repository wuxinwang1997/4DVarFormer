...
def load_ndf(year, time_scale, variables):
    ...
    for var in variables:
        if var in PRESSURE_LEVEL_VARS:
            if year == 2021:
                ps = glob.glob(os.path.join(path, f"202*.nc"))
            else:
...
def fetch_dataset(args, cursor_time, normalize_mean, normalize_std, variables):
    ...
    Xb_nc.to_netcdf(f"{path}/background_dir/{cursor_time.year}year_{cursor_time.month}month_pred{args.lead_time}day.nc")
    print(Xb_nc)
