...
from src.inference.inference import assim_cycle_inference, assim_cycle_4dvarnet, assim_cycle_vit
...
def assimilate_cycle_inference(data_dir,
    ...
    mean = np.zeros([1, 24, 1, 1])
    std = np.zeros([1, 24, 1, 1])
    for i, key in enumerate(normalize_mean):
        mean[:, i] = normalize_mean[key]
        std[:, i] = normalize_std[key]
    ...
    for i, ic in enumerate(ics):
        if da_method == "4dvarformer":
            seq_real, seq_assim, seq_rmse, seq_acc = assim_cycle_inference(ic,
                                                                        eval_dataset,
                                                                        module,
                                                                        afnonet,
                                                                        dt,
            ...
            seq_real, seq_assim, seq_rmse, seq_acc = assim_cycle_4dvarnet(ic,
                                                                           eval_dataset,
                                                                           module,
                                                                           afnonet,
                                                                           dt,
            ...
            seq_real, seq_assim, seq_rmse, seq_acc = assim_cycle_vit(ic,
                                                                    eval_dataset,
                                                                    module,
                                                                    afnonet,
                                                                    dt,
        ...
        if i == 0:
            ...
            val_rmse = seq_rmse
            val_acc = seq_acc
            val_xb = np.expand_dims(seq_xb, axis=0)
        else:
            ...
            val_xb = np.concatenate((val_xb, np.expand_dims(seq_xb, axis=0)), axis=0)
    ...
    # np.save(f"{output_dir}/xb_{da_method}_obs{obs_vars}_xb_{init_time}day_single.npy", val_xb * std + mean)
    # np.save(f"{output_dir}/real_{init_time}day_single.npy", val_real * std + mean)
    np.save(f"{output_dir}/assim_{da_method}_xb_{init_time}day_wo_obs.npy", val_assim * std + mean)
